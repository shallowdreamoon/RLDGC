...
from utils import save_result, get_action, load_mat_data, normalize_adj_1
...
def get_data(dataset):
    ...
    features, adj_ori, adj_norm, adj_lab, label = data_prepare(dataset_all[0], dataset)
...
def get_mat_data(dataset):
    G, X, Y = load_mat_data(dataset)
    adj_norm, adj_ori = normalize_adj_1(G)
    adj_ori = torch.Tensor(adj_ori.todense())
...
def task(cfg, dataset, policy = None):
    features, adj_ori, adj_norm, adj_lab, label, label_indices, att_sim = get_mat_data(dataset)
    ...
    premodel_path = r"E:\study\pycharm\RLDGC\RLDGC\pretrain_res\ACM\premodel_ACM.pkl".format(dataset)
